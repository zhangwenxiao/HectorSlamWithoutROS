...
void SerialPort::Open ()
{
    ...
    // TODO:
    // Because the Win32 serial port API is a piece of crap, we have to
    // use overlapped files to get anywhere near the flexibility we get
    // with the POSIX API (such as decent timeout support, the ability
    // to wait for data to be available without blocking forever, etc).
    // The downside is that this makes the operations that are simple
    // for non-overlapped IO become nasty.
...
#else
    // WaitForDataOrTimeout() will do an initial check to see if there
    // is data available immediately and only select() if there isn't
    if (WaitForDataOrTimeout () == TIMED_OUT)
    {
        if (_debug >= 2)
        {

...
std::string RFCOMMPort::GetStatus () const
{
    ...
    status << "BT-specific status:" << endl;
    status << "Device ID: " << _devid << endl;
    status << "Device Port: " << (unsigned int) _port << endl;
...
//     addr.rc_channel = (uint8_t) _port;
//     str2ba(_devid.c_str(), &addr.rc_bdaddr);
...
//         Close ();
//         stringstream ss;
//         ss << "Failed to connect to " << _devid << ": (" << ErrNo () << ") " << StrError (ErrNo ());
//         throw PortException (ss.str ());
//     }
...
//     res.ai_family = AF_BLUETOOTH;
//     res.ai_socktype = SOCK_STREAM;
//     res.ai_protocol = BTPROTO_RFCOMM;

//     struct sockaddr_rc listen_addr = { 0 };
//     bdaddr_t bdaddr_any = { 0 };
//     listen_addr.rc_family = AF_BLUETOOTH;
//     listen_addr.rc_bdaddr = bdaddr_any;
//     listen_addr.rc_channel = (uint8_t) _port;
...
//     if (bind (_listenSock, (struct sockaddr*)&listen_addr, sizeof(listen_addr)) < 0)
//     {
//         Close ();
//         stringstream ss;
